...
public class BeaconDetailActivity extends Activity {
	...
	private BallistaBeacon mBeacon;
...
//            this.mBeacon = new Gson().fromJson(intent.getExtras().getString(BEACON_KEY, "{}"), BallistaBeacon.class);
			this.mBeacon = BeaconApp.getInstance().getBecons().get(
			...
			if (mBeacon != null) {
				...
				mProximityTv.setText("Distance : " + mBeacon.distance);
			} else {
				finish();
			}
		} else {
