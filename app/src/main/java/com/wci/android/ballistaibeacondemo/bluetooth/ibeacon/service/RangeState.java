...
public class RangeState {
    ...
    /**
     * Returns a list of iBeacons that are tracked,
     * and then removes any from the list that should not be there for the next cycle
     */
        ...
        synchronized (rangedIBeacons) {
            for (IBeacon iBeacon : rangedIBeacons.keySet()) {
                ...
                if (!rangedIBeacon.noMeasurementsAvailable() == true) {
                    ...
                    if (IBeaconManager.debug)
                        Log.d(TAG, "Dumping iBeacon from RangeState because it has no recent measurements.");
                }
            }
