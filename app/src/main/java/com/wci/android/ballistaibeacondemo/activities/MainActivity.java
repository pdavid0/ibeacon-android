...
public class MainActivity extends Activity implements BeaconConsumer,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		listBeaconRequest = new ListBeaconRequest(
				getString(R.string.ballista_api_list_beacons),
				this
		);
...
//        mBeaconManager.setForegroundBetweenScanPeriod(1000L * 5L); // 5 mins
	...
	protected void onResume() {
		...
		RequestManager.getInstance().performRequest(
				listBeaconRequest,
				MainActivity.this,
	...
	public void onBallistaBeaconItemClick(BallistaBeacon mBeacon) {
		...
		startActivity(
				...
				ActivityOptions.makeSceneTransitionAnimation(
						this,
						findViewById(R.id.list_item_payload),
						"payload"
				).toBundle()
		);
	}
...
//                }
	...
	public void didDetermineStateForRegion(int state, Region region) {
		Log.i(
				...
				"Did determine Region ID : "
						+ region.getUniqueId()
						+ "\tstate: " + ((state == MonitorNotifier.INSIDE)
						? " INSIDE "
						: " OUTSIDE ")
		);
	}
	...
	public void didRangeBeaconsInRegion(Collection<Beacon> beacons, Region region) {
		...
		BeaconApp.getBus().post(
				new HistoryEvent(
						new Event(
								"RangeNotifier", String.format(
								"Did Range Beacons In Region : %1$s, %2$s",
								region.getUniqueId(),
								sb.toString()
						), ""
						)
