package com.wci.android.ballistaibeacondemo.bluetooth;
...
public class BluetoothCrashResolver {
     ...
     * //
     * // While it is possible to manually turn bluetooth_old off then back on in
     ...
     * disctinct bluetooth_old devices scanned, and prevent crashes before they happen.
     * <p/>
     * This works very well if the app containing this class is the only one running bluetooth_old
    ...
    public void notifyScannedDevice(BluetoothDevice device, BluetoothAdapter.LeScanCallback scanner) {
        ...
        if (isDebugEnabled()) {
            ...
            if (oldSize != newSize && newSize % 100 == 0) {
                if (isDebugEnabled())
                    Log.d(TAG, "Distinct bluetooth_old devices seen: " + distinctBluetoothAddresses.size());
            }
        }
        if (distinctBluetoothAddresses.size() > getCrashRiskDeviceCount()) {
            if (PREEMPTIVE_ACTION_ENABLED && !recoveryInProgress) {
                Log.w(TAG, "Large number of bluetooth_old devices detected: " + distinctBluetoothAddresses.size() + " Proactively attempting to clear out address list to prevent a crash");
    ...
    public void crashDetected() {
        ...
        if (distinctBluetoothAddresses.size() > 0) {
            if (isDebugEnabled())
                Log.d(TAG, "Distinct bluetooth_old devices seen at crash: " + distinctBluetoothAddresses.size());
        }
        ...
        if (recoveryInProgress) {
            if (isDebugEnabled())
                Log.d(TAG, "Ignoring bluetooth_old crash because recovery is already in progress.");
        } else {
            startRecovery();
        }
        processStateChange();
    ...
    private void startRecovery() {
        // The discovery operation will start by clearing out the bluetooth_old mac list to only the 256
        ...
        if (!adapter.isDiscovering()) {
            ...
            if (!adapter.startDiscovery()) {
                Log.w(TAG, "Can't start discovery.  Is bluetooth_old turned on?");
            }
            if (isDebugEnabled())
            ...
            if (TIME_TO_LET_DISCOVERY_RUN_MILLIS > 0) {
                if (isDebugEnabled())
                    Log.d(TAG, "We will be cancelling this discovery in " + TIME_TO_LET_DISCOVERY_RUN_MILLIS + " milliseconds.");
    ...
    private void saveState() {
        ...
        if (isDebugEnabled())
            Log.d(TAG, "Wrote " + distinctBluetoothAddresses.size() + " bluetooth_old addresses");
    ...
    private void loadState() {
        ...
        if (isDebugEnabled())
            Log.d(TAG, "Read " + distinctBluetoothAddresses.size() + " bluetooth_old addresses");
    }
